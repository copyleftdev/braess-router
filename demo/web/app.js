...
(() => {
  ...
  const names = {task_queued:'Queued',request_started:'Request sent',response_received:'Response received',task_completed:'Completed',task_uncertain:'Uncertain'};
  ...
  function routeOf(task){return response(task)?.route || (state(task)==='task_uncertain'?'uncertain':null);}
  ...
  function render(){
    ...
    let completed=0,uncertain=0,pending=0;
    for(const task of tasks){
      ...
      button.querySelector('small').textContent=(r?.route||(s==='task_uncertain'?'No route reported':'Awaiting route'))+' · '+(names[s]||'Not started');
      button.querySelector('.row-time').textContent=r?Number(r.elapsed_ms).toFixed(2)+' ms':'—';
      if(s==='task_completed')completed++;else if(s==='task_uncertain')uncertain++;else if(s!=='not_started')pending++;
    }
    $('completed').textContent=completed;$('uncertain').textContent=uncertain;$('pending').textContent=pending;
  ...
  function inspect(){
    ...
    $('selected-description').textContent=s==='task_uncertain'?'Completion was not confirmed. This task is retained as uncertain.':r?.route==='fallback'?'Braess returned a local fallback. No handler completion is implied.':s==='task_completed'?'The gateway returned a handler result. This fixture does not measure legal-review accuracy.':'Only events up to the replay clock are shown.';
    ...
    $('provenance').textContent='Run '+bundle.run.run_id+' · Source '+task.id+' · Last visible event SHA-256 '+(events.at(-1)?.sha256||'not observed');
  }
  function check(data){
  ...
  async function load(){
    try{
      ...
      $('task-total').textContent=tasks.length+' text fixtures';
